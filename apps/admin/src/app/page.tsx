...
async function readCompleteList(
  ...
  if (limit === 1) {
    return seed as ProductList | OrderList;
  }
...
export default async function AdminHomePage() {
  ...
  const recentEvents = data.activityReport.recentEvents.slice(0, 6);
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <article className="rounded-[2rem] border border-slate-200 bg-[linear-gradient(180deg,_#ffffff_0%,_#f8fbff_100%)] p-6 shadow-[0_18px_60px_rgba(15,23,42,0.05)]">
          ...
          <h3 className="mt-1 text-xl font-semibold text-slate-950">
            Recent enterprise log
          </h3>

          <div className="mt-6 max-h-[40rem] overflow-y-auto pr-1 space-y-6">
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="max-w-[8rem] text-right text-xs text-slate-400">
                        ...
                        <p className="mt-1 break-words font-medium text-slate-500">
                          {event.actorLabel}
                        </p>
                      </div>
                    </div>
