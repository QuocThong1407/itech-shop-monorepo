...
import { deleteOrder, fetchOrderDetail, fetchOrders, updateOrderStatus } from "../api";
import {
  buildOrderStats,
  filterOrders,
...
export function useOrdersPage() {
  ...
  const handleStatusChange = async (order: OrderRecord, nextStatus: string) => {
    ...
    const ok = window.confirm(`Change order status to ${nextStatus.toLowerCase()}?`);
    if (!ok) return;
    ...
    try {
      ...
      if (message.includes("modified by another request") || message.includes("409")) {
        let latest: OrderRecord | null = null;

        try {
          latest = await fetchOrderDetail(order.id);
        } catch {
          latest = null;
        }
        ...
        if (latest && selectedOrder?.id === order.id) {
          setSelectedOrder(latest);
        }

        if (latest && normalizeStatus(latest.status) === normalizeStatus(nextStatus)) {
          return;
        }

        if (latest) {
          const allowed = getAvailableStatusOptions(latest.status);
        ...
        window.alert("Order data changed on the server. The latest state has been reloaded.");
      } else {
  ...
  const handleDelete = async (order: OrderRecord) => {
    const ok = window.confirm("Delete this order? This should only be used for pending orders.");
    if (!ok) return;
