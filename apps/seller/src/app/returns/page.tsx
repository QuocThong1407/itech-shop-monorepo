...
export default function SellerReturnsPage() {
  ...
  const loadReturns = async () => {
    setLoading(true);
    try {
      const result = await apiJson<ReturnsResponse>(
        "/returns?page=1&limit=1000",
      );
  ...
  const updateStatus = async (record: ReturnRecord, nextStatus: string) => {
    ...
    try {
      await apiJson(`/returns/${record.id}/status`, {
        method: "PATCH",
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
                  Review return requests for your products, inspect order
                  details, and move them through the approval flow.
                </p>
              </div>
            </div>
          </div>
