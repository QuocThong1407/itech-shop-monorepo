...
export default function ProductViewModal({
  ...
  const meta = getStatusMeta(selectedStatus);

  return (
    <ModalShell
      open={open}
      title={product?.name ?? "Product detail"}
      subtitle="Inspect product media, pricing, seller information and variant inventory."
      onClose={onClose}
        ...
        <div className="grid gap-0 lg:grid-cols-[1fr_0.98fr]">
          <div className="space-y-5 p-6 lg:border-r lg:border-slate-200">
            ...
            <KeyValueGrid
              items={[
                ...
                { label: "Reviews", value: (product.reviewCount ?? 0).toLocaleString("vi-VN") },
          ...
          <div className="space-y-5 bg-slate-50 p-6">
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="text-sm font-semibold text-slate-900">
                            {formatVariantAttributes(variant.variantAttributes)}
                          </p>
                          <p className="mt-1 text-xs text-slate-500">
                            Quantity {variant.quantity} - Price adj. {variant.priceAdjustment ?? 0}
                          </p>
                        </div>
            ...
            <div className="flex justify-end gap-3">
              <Button onClick={() => onEdit(product)} variant="secondary" className="!shadow-none">
                Edit product
              </Button>
            </div>
          </div>
