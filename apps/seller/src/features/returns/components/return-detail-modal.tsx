...
import type { ReturnCustomerUser, ReturnPayment, ReturnRecord } from "../types";

type ReturnDetailModalProps = {
  open: boolean;
  actionLoading: boolean;
  selectedRecord: ReturnRecord | null;
  customer: ReturnCustomerUser | undefined;
  payment: ReturnPayment | undefined;
  items: NonNullable<NonNullable<ReturnRecord["Order"]>["OrderItem"]>;
  onClose: () => void;
  onStatusChange: (record: ReturnRecord, nextStatus: string) => void;
};
...
export default function ReturnDetailModal({
  ...
  return (
    <ModalShell
      open={open}
      eyebrow="Seller returns"
      title={
        selectedRecord
          ? `Return #${selectedRecord.id.slice(0, 10).toUpperCase()}`
          : "Return detail"
      }
      subtitle="Review customer return details, linked payment context, and returned items."
      onClose={onClose}
      ...
      {selectedRecord ? (
        <div className="grid gap-0 lg:grid-cols-[1fr_0.98fr]">
          ...
          <div className="space-y-5 bg-slate-50 p-6">
            ...
            <DetailSection title="Seller actions" className="shadow-sm">
              <div className="mt-4 text-sm text-slate-500">
                Use the status selector to move this return through your fulfillment workflow.
              </div>
            </DetailSection>
          </div>
        </div>
